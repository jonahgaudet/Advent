...
computer system that runs this port isn't compatible with the docking program on the ferry, so the docking parameters
...
000000000000000000000000000001001001  (decimal 73) So, because of the mask, the value 73 is written to memory address
...
000000000000000000000000000001100101  (decimal 101) This time, the mask has no effect, as the bits it overwrote were
already the values the mask tried to set. Finally, the program tries to write 0 to address 8:
...
If the bitmask bit is 0, the corresponding memory address bit is unchanged. If the bitmask bit is 1,
the corresponding memory address bit is overwritten with 1. If the bitmask bit is X, the corresponding memory address
bit is floating. A floating bit is not connected to anything and instead fluctuates unpredictably. In practice,
this means the floating bits will take on all possible values, potentially causing many memory addresses to be
written all at once!
...
address: 000000000000000000000000000000101010  (decimal 42) mask:    000000000000000000000000000000X1001X result:
000000000000000000000000000000X1101X After applying the mask, four bits are overwritten, three of which are
different, and two of which are floating. Floating bits take on every possible combination of values; with two
...
000000000000000000000000000000010000  (decimal 16) 000000000000000000000000000000010001  (decimal 17)
000000000000000000000000000000010010  (decimal 18) 000000000000000000000000000000010011  (decimal 19)
000000000000000000000000000000011000  (decimal 24) 000000000000000000000000000000011001  (decimal 25)
000000000000000000000000000000011010  (decimal 26) 000000000000000000000000000000011011  (decimal 27) The entire
36-bit address space still begins initialized to the value 0 at every address, and you still need the sum of all
values left in memory at the end of the program. In this example, the sum is 208.
...
def assignValuesToMemory(lines):
    currentMask = ""
    total = 0
    values = {}
    for line in lines:
        if "[" not in line:
            ...
            toStore = int(line.split(" = ")[-1])
            xLocations = []
            value = int(line.split("[")[-1].split("]")[0])
            # print(value)
            for i in range(0, len(currentMask)):
                bitPosition = len(currentMask) - 1 - i
                valAt = currentMask[i]
                # print(bitPosition, valAt)
                if valAt == "1":
                    value = set_bit(value, bitPosition)
                elif valAt == "X":
                    value = clear_bit(value, bitPosition)
                    xLocations.append(bitPosition)

            # print(xLocations)
            allValues = getValidAddresses(value, xLocations, 0, [])
            # print(allValues)
            for v in allValues:
                values[v] = toStore
...
def getValidAddresses(value, xLocations, currentIndex, alreadyObtained=[]):

    if currentIndex == 35:
        if currentIndex in xLocations:
            value = set_bit(value, currentIndex)
            # print(bin(value))
            alreadyObtained.append(value)
            value = clear_bit(value, currentIndex)
        # print(bin(value))
        alreadyObtained.append(value)
    ...
    if currentIndex in xLocations:
        value = set_bit(value, currentIndex)
        alreadyObtained = getValidAddresses(value, xLocations, currentIndex+1, alreadyObtained)
        value = clear_bit(value, currentIndex)
    alreadyObtained = getValidAddresses(value, xLocations, currentIndex+1, alreadyObtained)
...
def getSumOfMasks(lines):
    ...
    for line in lines:
        if "[" not in line:
            ...
            memoryLocation = int(line.split("[")[-1].split("]")[0])
            for i in range(0, len(currentMask)):
                bitPosition = len(currentMask) - 1 - i
                valAt = currentMask[i]
                if valAt == "X":
...
if __name__ == '__main__':
    lines = getLinesFromFile("InputFiles\input_Day14.txt")
    print(getSumOfMasks(lines))
    # Answer is 14722016054794
    print(assignValuesToMemory(lines))
    # Answer is 3618217244644
