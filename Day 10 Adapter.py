...
Find a chain that uses all of your adapters to connect the charging outlet to your device's built-in adapter and count the
    joltage differences between the charging outlet, the adapters, and your device. What is the number of 1-jolt differences
...
You glance back down at your bag and try to remember why you brought so many adapters; there must be more than a trillion
    valid ways to arrange them! Surely, there must be an efficient way to count the arrangements.
...
def partB(adapters):
    ...
    for i in range(0, len(adapters) - 3):
        if adapters[i] + 3 == adapters[i+1]:
            value = getPossibilities(adapters[lastChoice:i+1], 0)
            print(value, adapters[lastChoice:i + 1])
            runningTotal *= value
            lastChoice = i+1

    value = getPossibilities(adapters[lastChoice:], 0)
    print(value, adapters[lastChoice:])
    return runningTotal * value


def getPossibilities(adapters, index):
    if index == len(adapters) - 1:
        return 1

    # print(index, adapters[index])
    total = 0
    for i in range(index + 1, len(adapters)):
        if adapters[i] - adapters[index] <= 3:
            total += getPossibilities(adapters, i)
        else:
            break
...
if __name__ == '__main__':
    lines = getLinesFromFile("InputFiles\input_Day10.txt")
    print(partA(lines))
    # Answer is 2812
    lines = getLinesFromFile("InputFiles\input_Day10.txt")
    print(partB(lines))
    # 6718464 is too low
    # 117440512 is too low
    # Answer is 386869246296064
    # Also written as 386,869,246,296,064
    # ...or 386 trillion, 869 billion, 246 million, 296 thousand and 64
    # gah dam
