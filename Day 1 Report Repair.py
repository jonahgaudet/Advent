...
The tropical island has its own currency and is entirely cash-only. The gold coins used there have a little picture of a
    starfish; the locals just call them stars. None of the currency exchanges seem to have heard of them, but somehow, you'll
    need to find fifty of these coins by the time you arrive so you can pay the deposit on your room.
...
def sum2(lines):
    for x in range(len(lines)):
        if lines[x] < ELEMENTS_SUM:
            for y in range(len(lines)):
                if lines[x] + lines[y] == ELEMENTS_SUM:
            ...
            for y in range(len(lines)):
                if lines[x] + lines[y] < ELEMENTS_SUM:
                    for z in range(len(lines)):
                        if lines[x] + lines[y] + lines[z] == ELEMENTS_SUM:
                            print(lines[x], lines[y], lines[z])
                            return lines[x] * lines[y] * lines[z]


def getIntLinesFromFile (filename):
...
if __name__ == '__main__':
    lines = getIntLinesFromFile("InputFiles\input_Day1.txt")
